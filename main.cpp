...
int main()
{
    ...
    try {
        ...
        Config::GetInstance().Initialize("config.cfg", GameCamera, ESP);
        ...
        Trigger = new TriggerBot(Myself, Players, GameCamera, mem.GetKeyboard(), Map);
        Config::GetInstance().Attach(Trigger, mem.GetKeyboard());

        std::thread coreThread(UpdateCore);
        std::thread renderThread(Render, Myself, Players, GameCamera, Spectators, ESP, Trigger);
        coreThread.join();
    }
    catch (...) {}
