...
// kernel click path removed

bool c_keys::SafeTriggerShot() { return true; }

bool c_keys::IsReactionTimeValid() {
    auto now = std::chrono::steady_clock::now();
    auto timeSinceLastTrigger = std::chrono::duration_cast<std::chrono::milliseconds>(now - lastTrigger);
    
    // Human reaction time: 150-400ms between shots
    int minReactionTime = 90 + (rand() % 250);
    
    if (timeSinceLastTrigger.count() >= minReactionTime) {
        lastTrigger = now;
        return true;
    }
    return false;
