...
{
	if (VMMDLL_MemVirt2Phys(this->vHandle, this->current_process.PID, va, &pa)) {
		return true;
	}
	return false;
}

bool Memory::VirtToPhys(uint64_t va, uint64_t& pa, int pid)
{
    if (pid == 0) pid = this->current_process.PID;
    if (VMMDLL_MemVirt2Phys(this->vHandle, pid, va, &pa)) {
        return true;
    }
    return false;
}
...
{
	if (!VMMDLL_MemWrite(this->vHandle, -1, address, static_cast<PBYTE>(buffer), size))
	{
