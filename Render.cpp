...
void Render(LocalPlayer* Myself, std::vector<Player*>* Players, Camera* GameCamera, Spectator* Spectators, Sense* ESP, TriggerBot* Trigger)
{
    ...
    while (true)
    {
        ...
        if (ImGui::Checkbox("Enable TriggerBot", &enabled)) {
            Trigger->SetEnabled(enabled);
            Config::GetInstance().Save();
        }
        float maxDist = Trigger->GetMaxDistance();
        ImGui::Text("Max Distance (m):");
        if (ImGui::SliderFloat("##TriggerMaxDist:", &maxDist, 50.0f, 400.0f, "% .0f")) {
            Trigger->SetMaxDistance(maxDist);
            Config::GetInstance().Save();
        }
        int trigDelay = Trigger->GetTriggerDelayMs();
        ImGui::Text("Trigger Delay (ms):");
        if (ImGui::SliderInt("##TriggerDelay:", &trigDelay, 0, 150)) {
            Trigger->SetTriggerDelayMs(trigDelay);
            Config::GetInstance().Save();
        }
        
        bool debugMode = Trigger->GetDebugMode();
        ImGui::Text("Debug Mode:");
        if (ImGui::Checkbox("##DebugMode:", &debugMode)) {
            Trigger->SetDebugMode(debugMode);
            Config::GetInstance().Save();
        }
        uint32_t trigKey = Trigger->GetTriggerKey();
        int trigKeyIdx = FindCurrentSelectionIndex(trigKey, keyValues, keyCount);
        ImGui::Text("Trigger Key:");
        if (ImGui::Combo("##TriggerKey", &trigKeyIdx, keyNames, keyCount)) {
            Trigger->SetTriggerKey(keyValues[trigKeyIdx]);
            Config::GetInstance().Save();
        }
